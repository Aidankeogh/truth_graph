...
found_note = lambda : random() < note_finding_probability # Returns true 50% of the time. 
...
for day in range(days):
    ...
    for explorer, ax in zip(explorers, axs):
        ...
        trusted_notes = {k: v for k, v in explorer.notes.items() if trusted_model._score(v) > bullshit_threshold}
        trusted_locations = torch.cat([explorer.wolf_sightings] + list(trusted_notes.values()))
...
for day in range(days): # Main loop
    ...
    for explorer in explorers: # in Env.step()
        ...
        explorer.wolf_sightings = torch.cat([explorer.wolf_sightings, new_sightings]) # in Env.step()
