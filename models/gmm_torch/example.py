...
def plot(data, y):
    ...
    for i, point in enumerate(data.data):
        if i <= n // 2:
            ...
            ax.scatter(*point, color="#000000", s=3, alpha=0.75, zorder=n + i)
        else:
            ax.scatter(*point, color="#ffffff", s=3, alpha=0.75, zorder=n + i)

        if y[i] == 0:
            # .. as well as their predicted class
            ax.scatter(
                *point, zorder=i, color="#dbe9ff", alpha=0.6, edgecolors=colors[1]
            )
        else:
            ax.scatter(
                *point, zorder=i, color="#ffdbdb", alpha=0.6, edgecolors=colors[5]
            )
