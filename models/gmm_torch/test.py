...
class CpuCheck(unittest.TestCase):
    ...
    def testEmMatchesDiagSkLearn(self):
        ...
        model = GaussianMixture(
            n_components, d, var_init=var_init, covariance_type="diag"
        )
        model_sk = sklearn.mixture.GaussianMixture(
            ...
            precisions_init=np.squeeze(1.0 / np.sqrt(var_init.data.numpy())),
        )
        ...
        np.testing.assert_almost_equal(
            np.squeeze(log_prob.data.numpy()), log_prob_sk, decimal=2, verbose=True
        )
        ...
        np.testing.assert_almost_equal(
            np.squeeze(log_resp.data.numpy()), log_resp_sk, decimal=0, verbose=True
        )
        ...
        # Test whether pi ..
        np.testing.assert_almost_equal(
            np.squeeze(pi.data.numpy()), pi_sk, decimal=1, verbose=True
        )

        # .. mu ..
        np.testing.assert_almost_equal(
            np.squeeze(mu.data.numpy()), mu_sk, decimal=1, verbose=True
        )

        # .. and var are approximately equal
        np.testing.assert_almost_equal(
            np.squeeze(var.data.numpy()), var_sk, decimal=1, verbose=True
        )
